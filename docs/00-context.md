...
| AC-7 | Every dispute receives a `recommendedAction` (first match wins): (1) days < 0 → ACCEPT (expired); (2) days ≤ 3 AND prob ≥ 40 AND amount ≥ $50 → URGENT_REVIEW; (3) prob < 40 OR amount < $50 → ACCEPT; (4) prob ≥ 60 → CONTEST; (5) else → ACCEPT. Note: amount < $50 guard on Rule 2 prevents URGENT_REVIEW escalation for trivial amounts. | unit test covering each branch including trivial-amount trap | Intelligence 25pts |
